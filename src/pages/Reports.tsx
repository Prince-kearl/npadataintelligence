...
export default function Reports() {
  ...
  const guard = () => {
    if (!allowed) {
      toast.error("Your role does not have permission to export data.");
      return false;
    }
    return true;
  };
  ...
  const exportCSV = () => {
    if (!guard()) return;
    const content = incidentsToCSV(mockIncidents);
    const name = timestampedName("npa_incidents", "csv");
    downloadBlob(name, content, "text/csv;charset=utf-8");
    logExport(name, "CSV", content);
    toast.success(`Exported ${mockIncidents.length} records as CSV`);
  };

  const exportExcel = () => {
    if (!guard()) return;
    // CSV with .xls extension opens cleanly in Excel without adding a heavy dependency
    const content = incidentsToCSV(mockIncidents);
  ...
  const exportSQL = () => {
    if (!guard()) return;
    const content = incidentsToSQLDump(mockIncidents);
    const name = timestampedName("npa_incidents", "sql");
    downloadBlob(name, content, "application/sql");
    logExport(name, "SQL Dump", content);
    toast.success("SQL dump generated from incident repository");
  };

  const exportPDF = () => {
    if (!guard()) return;
    // Lightweight printable HTML — opens print dialog, user can Save as PDF
    const win = window.open("", "_blank");
    if (!win) return;
    const rows = mockIncidents
      .map(
        (i) =>
          `<tr><td>${i.id}</td><td>${i.incident_date}</td><td>${i.region}</td><td>${i.category}</td><td>${i.status}</td></tr>`
      )
      .join("");
    win.document.write(`<html><head><title>NPA Incident Report</title>
      ...
      </head><body><h1>NPA Incident Summary Report</h1><p>Generated ${new Date().toLocaleString()}</p>
      <table><thead><tr><th>ID</th><th>Date</th><th>Region</th><th>Category</th><th>Status</th></tr></thead><tbody>${rows}</tbody></table>
